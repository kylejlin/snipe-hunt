...
import { Filter, Offset } from "./bitwiseUtils";
...
export function getMinimalGameStateAnalyzer(
  initState: MinimalGameState
): MinimalGameAnalyzer {
  let state = initState;
  ...
  function tryPerform(
    atomic: Atomic
  ): Result<MinimalGameState, IllegalGameStateUpdate> {
  ...
  function forcePerform(atomic: Atomic): MinimalGameState {
    if ("plyType" in atomic) {
      switch (atomic.plyType) {
        case PlyType.SnipeStep:
  ...
  function forcePerformSnipeStep(step: SnipeStep): MinimalGameState {
  ...
  function cloneState(src: MinimalGameState): MinimalGameState {
    return {
  ...
  function forcePerformDrop(drop: Drop): MinimalGameState {
    const reserve =
      state.turn === Player.Alpha
        ? CardLocation.AlphaReserve
        : CardLocation.BetaReserve;
  ...
  function forcePerformAnimalStep(step: AnimalStep): MinimalGameState {
  ...
  function setState(newState: MinimalGameState): void {
    state = newState;
  }
}
