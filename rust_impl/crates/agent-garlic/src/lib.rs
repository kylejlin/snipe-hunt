//! Garlic: a deterministic, allocation-conscious Snipe Hunt searcher.
//!
...
const ACTIVATION_KNOWN: u16 = 1 << 14;
const ACTIVATES: u16 = 1 << 15;
...
const ACTIVATION_REQUIREMENTS: [[ActivationRequirement; 2]; 16] = activation_requirements();

const fn activation_requirements() -> [[ActivationRequirement; 2]; 16] {
    let mut result = [[ActivationRequirement::NONE; 2]; 16];
    let mut animal = 0;
    while animal < 16 {
        let actor = 1 << animal;
        ...
        let mut element = 0;
        while element < 4 {
            let pair = if UNARY_MASKS[element] & actor != 0 {
                Some((BINARY_MASKS[element], TERNARY_MASKS[element]))
            } else if BINARY_MASKS[element] & actor != 0 {
                Some((UNARY_MASKS[element], TERNARY_MASKS[element]))
            } else if TERNARY_MASKS[element] & actor != 0 {
                Some((UNARY_MASKS[element], BINARY_MASKS[element]))
            } else {
                None
            };
            if let Some((first, second)) = pair {
                assert!(requirement < 2, "animal has more than two activation roles");
                result[animal][requirement] = ActivationRequirement { first, second };
                requirement += 1;
            }
            element += 1;
        }
...
struct Ply(NonZeroU32);

impl Ply {
    #[cfg(test)]
    fn new(first: Action, second: Option<Action>) -> Self {
        Self::from_packed(pack_action(first), second.map(pack_action))
    }

    fn from_packed(first: u16, second: Option<u16>) -> Self {
        let packed = u32::from(first) | (u32::from(second.unwrap_or(0)) << 16);
        Self(NonZeroU32::new(packed).expect("a ply always has a first action"))
    }

    fn first(self) -> Action {
        unpack_action(self.first_packed())
    }

    fn second(self) -> Option<Action> {
        self.second_packed().map(unpack_action)
    }

    fn first_packed(self) -> u16 {
        self.0.get() as u16
    }

    fn second_packed(self) -> Option<u16> {
        let packed = (self.0.get() >> 16) as u16;
...
fn pack_action(action: Action) -> u16 {
    match action {
        Action::AnimalStep(step) => {
            1 | ((step.actor as u16) << 2)
                | (u16::from(step.direction == StepDirection::Retreat) << 6)
                | ((step.destination as u16) << 7)
        }
        Action::SnipeStep(step) => 2 | ((step.destination as u16) << 2),
        Action::Drop(drop) => 3 | ((drop.actor as u16) << 2) | ((drop.destination as u16) << 6),
    }
}

fn pack_generated_action(action: Action, activates: bool) -> u16 {
    pack_action(action) | ACTIVATION_KNOWN | if activates { ACTIVATES } else { 0 }
}

fn unpack_action(packed: u16) -> Action {
    match packed & 3 {
        1 => Action::AnimalStep(AnimalStep {
            actor: ANIMALS[((packed >> 2) & 0xf) as usize],
            direction: if packed & (1 << 6) == 0 {
                StepDirection::Advance
            } else {
                StepDirection::Retreat
            },
            destination: RANKS[((packed >> 7) & 7) as usize],
        }),
        2 => Action::SnipeStep(SnipeStep {
            destination: RANKS[((packed >> 2) & 7) as usize],
        }),
        3 => Action::Drop(AnimalDrop {
            actor: ANIMALS[((packed >> 2) & 0xf) as usize],
            destination: RANKS[((packed >> 6) & 7) as usize],
        }),
        _ => unreachable!("packed ply actions are nonzero"),
    }
}

struct ActionBuffer {
    actions: [MaybeUninit<u16>; ACTION_CAPACITY],
    len: usize,
}
...
impl ActionBuffer {
    fn new() -> Self {
        Self {
            actions: [const { MaybeUninit::uninit() }; ACTION_CAPACITY],
            len: 0,
        }
    }
    ...
    fn as_slice(&self) -> &[u16] {
        // Every element below `len` was initialized by `push`, and `u16` needs no drop.
        unsafe { slice::from_raw_parts(self.actions.as_ptr().cast(), self.len) }
    }
...
impl PackedActionWriter for ActionCounter {
    fn push(&mut self, _action: u16) {
        self.len += 1;
    }
...
impl Position {
    ...
    fn write_legal_actions<W: PackedActionWriter>(self, writer: &mut W) {
    ...
    fn write_snipe_steps<W: PackedActionWriter>(self, writer: &mut W) {
        ...
        if let Some(destination) = advance_destination(source, self.active_player) {
            writer.push(pack_generated_action(
    ...
    fn write_drops<W: PackedActionWriter>(self, writer: &mut W) {
        if self.reserve().animal_total(self.active_player) <= 1 {
            return;
        }
        let mut animals = self.reserve().presence(self.active_player);
        while animals != 0 {
            let animal = take_animal(&mut animals);
    ...
    fn write_animal_steps<W: PackedActionWriter>(self, writer: &mut W) {
        for source in RANKS {
            let source_pile = self.rank(source);
            let mut animals = source_pile.presence(self.active_player);
            while animals != 0 {
                let animal = take_animal(&mut animals);
                let friendly_count = 1 + u8::from(source_pile.twins & animal_bit(animal) != 0);
                if let Some(destination) = advance_destination(source, self.active_player) {
                    let step = AnimalStep {
                        actor: animal,
                    ...
                    if let Some(activates) =
                        self.generated_animal_step_activation(source, friendly_count, step)
                    {
                        writer.push(pack_generated_action(Action::AnimalStep(step), activates));
                    }
                }
    ...
    fn generated_animal_step_activation(
        self,
        source: Rank,
        friendly_count: u8,
        step: AnimalStep,
    ...
    #[cfg(test)]
    fn apply_generated(self, action: Action) -> Self {
        self.apply_packed_action(pack_action(action))
    }

    fn apply_packed_action(mut self, action: u16) -> Self {
        let player = self.active_player;
        match action & 3 {
            2 => {
                let destination = RANKS[((action >> 2) & 7) as usize];
                let source = self
                    .snipe_rank(player)
                    .expect("generated snipe step has a source");
                self.rank_mut(source).remove_snipe(player);
                self.rank_mut(destination).add_snipe(player);
                self.active_player = player.opponent();
            }
            3 => {
                let actor = ANIMALS[((action >> 2) & 0xf) as usize];
                let destination = RANKS[((action >> 6) & 7) as usize];
                self.reserve_mut().remove_animal(actor, player);
                self.rank_mut(destination).add_animal(actor, player);
                self.active_player = player.opponent();
            }
            1 => {
                let actor = ANIMALS[((action >> 2) & 0xf) as usize];
                let direction = if action & (1 << 6) == 0 {
                    StepDirection::Advance
                } else {
                    StepDirection::Retreat
                };
                let destination = RANKS[((action >> 7) & 7) as usize];
                let source = source_for_destination(destination, self.active_player, direction)
                    .expect("generated animal step has a source");
                let destination_before = self.rank(destination);
                let activates = if action & ACTIVATION_KNOWN != 0 {
                    action & ACTIVATES != 0
                } else {
                    would_activate(actor, destination_before)
                };
                self.rank_mut(source).remove_animal(actor, player);
                if activates {
                    self.capture_into_reserve(destination_before);
                    *self.rank_mut(destination) = Pile::EMPTY;
                }
                self.rank_mut(destination).add_animal(actor, player);
                if self.leading_action.is_some() {
                    self.leading_action = None;
                    ...
                    self.leading_action = Some(AnimalStep {
                        actor,
                        direction,
                        destination,
                    });
            ...
            _ => unreachable!("packed actions are nonzero"),
        }
        self
    }
    ...
    fn capture_into_reserve(&mut self, captured: Pile) {
        ...
        let captured_animals = captured.alpha | captured.beta;
        let captured_pairs = captured.twins | (captured.alpha & captured.beta);
        let captured_singles = captured_animals & !captured_pairs;
        let reserve = self.reserve_mut();
        let existing = reserve.presence(capturer);
        reserve.twins |= captured_pairs | (captured_singles & existing);
        *reserve.presence_mut(capturer) |= captured_animals;
        reserve.snipes |= captured.snipes;
    }
    ...
    fn winner(self) -> Option<Player> {
        ...
        let mut actions = ActionCounter::default();
        self.write_legal_actions(&mut actions);
        (actions.len == 0).then_some(self.active_player.opponent())
    }
}
...
fn take_animal(animals: &mut u16) -> Animal {
    let index = animals.trailing_zeros() as usize;
    *animals &= *animals - 1;
    ANIMALS[index]
}
...
fn would_activate(actor: Animal, destination: Pile) -> bool {
    ...
    let [first, second] = ACTIVATION_REQUIREMENTS[actor as usize];
    activation_requirement_met(first, present) || activation_requirement_met(second, present)
}

fn activation_requirement_met(requirement: ActivationRequirement, present: u16) -> bool {
    requirement.first & present != 0 && requirement.second & present != 0
}
...
impl FastHasher {
    fn mix(&mut self, value: u64) {
        self.value = (self.value.rotate_left(5) ^ value).wrapping_mul(0x517C_C1B7_2722_0A95);
    }
}
...
impl Default for GarlicAnalyzer {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Analyzer for GarlicAnalyzer {
    ...
    fn is_fully_solved(&self) -> Option<OptimalOutcome> {
        // Garlic publishes a mate only after completing a full-window root search over every
...
fn generate_plies(state: Position, preferred: Option<Ply>, output: &mut Vec<ScoredPly>) {
    ...
    for &first in first_actions.as_slice() {
        let first_order = packed_action_order(state, first);
        if packed_action_is_standalone(first) {
            push_scored_ply(
                Ply::from_packed(first, None),
                preferred,
                first_order,
                output,
            );
        ...
        let after_first = state.apply_packed_action(first);
        ...
        for &second in second_actions.as_slice() {
            push_scored_ply(
                Ply::from_packed(first, Some(second)),
                preferred,
                first_order + packed_action_order(after_first, second),
                output,
            );
        }
    }
...
fn push_scored_ply(ply: Ply, preferred: Option<Ply>, base_order: i32, output: &mut Vec<ScoredPly>) {
    ...
    if ply.second_packed().is_some() {
        order += 100;
    }
    output.push(ScoredPly { ply, order });
}

fn packed_action_is_standalone(action: u16) -> bool {
    action & 3 != 1
}

fn packed_action_order(state: Position, action: u16) -> i32 {
    match action & 3 {
        1 => {
            let actor = ANIMALS[((action >> 2) & 0xf) as usize];
            let destination = state.rank(RANKS[((action >> 7) & 7) as usize]);
            let activates = if action & ACTIVATION_KNOWN != 0 {
                action & ACTIVATES != 0
            } else {
                would_activate(actor, destination)
            };
            if activates {
...
fn apply_ply(state: Position, ply: Ply) -> Option<Position> {
    let mut child = state.apply_packed_action(ply.first_packed());
    if let Some(second) = ply.second_packed() {
        child = child.apply_packed_action(second);
    }
    Some(child)
}
...
fn mate_in(winner: Player, plies: u32) -> Evaluation {
    MateInN::new(winner, plies)
        .expect("Garlic's mate distance is supported by snipe-core")
        .into()
}

fn estimate(millipoints: i32) -> Evaluation {
    EvaluationEstimate::from_millipoints(millipoints)
        .expect("Garlic clamps heuristic evaluations to the public range")
        .into()
}
...
31b. Horse &2";
    ...
    fn test_plies(state: &State) -> Vec<(Ply, State)> {
        ...
        for first in first_actions {
            ...
            if after_first.active_player != player || after_first.winner().is_some() {
                result.push((Ply::new(first, None), after_first));
                continue;
            }
            ...
            for second in second_actions {
                ...
                result.push((Ply::new(first, Some(second)), child));
            }
        }
        result
    }
    ...
    fn packed_rules_match_the_reference_across_random_play() {
        assert_eq!(std::mem::size_of::<Ply>(), 4);
        assert_eq!(std::mem::size_of::<Option<Ply>>(), 4);
        assert_eq!(std::mem::size_of::<ScoredPly>(), 8);
        for seed in 0..32 {
            ...
            for _ in 0..160 {
                ...
                for &action in &reference_actions {
                    assert_eq!(Ply::new(action, None).first(), action);
                    assert_eq!(Ply::new(action, Some(action)).second(), Some(action));
                }
                let generated_actions: Vec<_> = packed_actions
                    .as_slice()
                    .iter()
                    .copied()
                    .map(unpack_action)
                    .collect();
                assert_eq!(
                    generated_actions, reference_actions,
                ...
                let checked = packed.apply(action).expect("reference-legal action");
                assert_eq!(
                    packed.apply_generated(action),
                    checked,
                    "generated-action fast path diverged for seed {seed}"
                );
                packed = checked;
            }
        }
    ...
    fn bug_position_does_not_regress_to_a_false_mate() {
        ...
        if let Evaluation::MateInN(mate) = analyzer.evaluation() {
            assert!(
                ...
                "Garlic published an unforced {mate:?}"
            );
        }
    }
