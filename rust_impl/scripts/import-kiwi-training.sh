...
usage() {
    cat <<'EOF'
Usage: ./scripts/import-kiwi-training.sh ARCHIVE.tar.gz

Verify and import an archive made by export-kiwi-training.sh.
An existing training/kiwi-main is moved to training/backups/ first.
Stop Kiwi training before running this script.
EOF
}

fail() {
    printf 'import-kiwi-training: %s\n' "$*" >&2
    exit 1
}
...
if command -v pgrep >/dev/null 2>&1 &&
    ...
    fail "Kiwi training appears to be running; stop it before importing"
fi

mkdir -p "$TRAINING_DIR"
work_dir=$(mktemp -d "$TRAINING_DIR/.kiwi-import.XXXXXX")
backup_path=
cleanup() {
    status=$?
...
while IFS= read -r entry; do
    case "$entry" in
        ...
        MANIFEST.txt | SHA256SUMS | kiwi-main | kiwi-main/ | kiwi-main/*) ;;
...
unexpected=$(find "$work_dir/kiwi-main" ! -type f ! -type d -print 2>/dev/null || true)
...
grep -qx 'format=kiwi-training-export-v1' "$work_dir/MANIFEST.txt" ||
...
for file in "${required_files[@]}"; do
    [[ -s "$work_dir/kiwi-main/$file" ]] ||
        fail "archive is missing required run file: $file"
done
grep -qx 'purity=rules-only-fresh-seed' "$work_dir/kiwi-main/state.txt" ||
    fail "archive is missing Kiwi's rules-only fresh-seed marker"
grep -qx 'training_protocol=continuous-v1' "$work_dir/kiwi-main/state.txt" ||
    fail "archive is missing Kiwi's continuous-training protocol marker"
...
printf 'Checksums passed. Validating the imported run with kiwi-train...\n'
(
    cd "$RUST_ROOT"
    cargo run --release -p kiwi-train -- status \
        --run-dir "$work_dir/kiwi-main"
)
...
if [[ -e $RUN_DIR ]]; then
    ...
    mkdir -p "$backup_dir"
    backup_path="$backup_dir/kiwi-main-before-import-$timestamp"
    [[ ! -e $backup_path ]] || fail "backup path already exists: $backup_path"
    mv "$RUN_DIR" "$backup_path"
fi

mv "$work_dir/kiwi-main" "$RUN_DIR"

printf 'Kiwi training run imported to:\n%s\n' "$RUN_DIR"
