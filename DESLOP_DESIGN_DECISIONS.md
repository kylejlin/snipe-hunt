...
## Webapp integration expectations

The replacement boundary must be capable of supporting:

- new games and generated initial deals;
- pass-and-play and either computer-controlled side;
- legal-card and legal-destination highlighting;
- first-animal-step previews and second-step selection;
- immediate wins on a first animal step;
- complete move application and winner detection;
- history navigation at both ply and animal-subply boundaries;
- branching or alternative lines;
- saved-game restoration and history import/export;
- timed computer moves;
- live analysis with tick progress, an evaluation, and an optimal line of play.

These features may be implemented through redesigned WASM and frontend protocols. Compatibility with the current JSON schemas, physical card IDs, search metadata, or worker message shapes is not required.
