//! Eel: a wide residual policy/value MCTS agent.
//!
//! Eel keeps the rules-only policy/value shape of Cherry while increasing the
//! trainable trunk from 128 units and one residual layer to 256 units and four
//! residual layers. Its checkpoint format and optimizer are intentionally
//! self-contained.
...
impl Model {
    ...
    pub fn from_bytes(bytes: &[u8]) -> io::Result<Self> {
        if bytes.len() != HEADER_SIZE + PARAM_COUNT * 4 || bytes.get(..8) != Some(MAGIC) {
            return Err(invalid("invalid Eel checkpoint"));
        }
        ...
        {
            return Err(invalid("incompatible Eel checkpoint dimensions"));
        }
        let training_steps =
        ...
        if parameters.iter().any(|value| !value.is_finite()) {
            return Err(invalid("non-finite Eel checkpoint"));
        }
        Ok(Self {
            parameters,
            training_steps,
...
pub struct EelAnalyzer {
    state: Option<State>,
    model: Model,
    search: Option<Search>,
}

impl Default for EelAnalyzer {
    fn default() -> Self {
        Self::new()
    }
}
...
impl EelAnalyzer {
    pub fn new() -> Self {
        Self::with_model(Model::seeded(0xEE17_5EA5_2026_0001))
    }
...
fn mate_in(winner: Player, plies: u32) -> Evaluation {
    MateInN::new(winner, plies)
        .expect("Eel mate distance is within Core's range")
        .into()
}
...
fn estimate(value: f32) -> Evaluation {
    ...
    EvaluationEstimate::from_millipoints(millipoints)
        .expect("bounded Eel value is a valid estimate")
        .into()
}
...
pub mod training {
    ...
    impl Adam {
        ...
        pub fn to_bytes(&self) -> Vec<u8> {
            ...
            bytes.extend_from_slice(b"EELOP001");
        ...
        pub fn from_bytes(bytes: &[u8]) -> io::Result<Self> {
            let header = 20;
            if bytes.len() != header + PARAM_COUNT * 8 || bytes.get(..8) != Some(b"EELOP001") {
                return Err(invalid("invalid Eel optimizer checkpoint"));
            }
            let count =
                u32::from_le_bytes(bytes[8..12].try_into().expect("optimizer length checked"))
                    as usize;
            if count != PARAM_COUNT {
                return Err(invalid("incompatible Eel optimizer dimensions"));
            }
            let step =
            ...
            if floats.iter().any(|value| !value.is_finite()) {
                return Err(invalid("non-finite Eel optimizer checkpoint"));
            }
            Ok(Self {
...
mod tests {
    ...
    fn analyzer_returns_a_complete_legal_ply() {
        for seed in 0..8 {
            ...
            let mut analyzer = EelAnalyzer::new();
