...
import { GamePosition, Network } from ".";
...
const ROLLOUTS_PER_MOVE = 1600;
const TEMPERATURE = 4;
const GAMES_PER_EPOCH = 1000;

export function generateTrainingData(network: Network): GamePosition[] {
  const positions: GamePosition[] = [];
  const analyzer = getMctsAnalyzer(network, NODE_SIZE_IN_I32S * 1e7);
  for (let i = 0; i < GAMES_PER_EPOCH; i++) {
    positions.push(...generateTrainingDataForOneGame(analyzer));
  }
  return positions;
}

function generateTrainingDataForOneGame(
  analyzer: MctsAnalyzer
): GamePosition[] {
  analyzer.reset(getRandomGameState());

  const positions: GamePosition[] = [];

  while (!analyzer.isGameOver()) {
    for (let i = 0; i < ROLLOUTS_PER_MOVE; i++) {
      analyzer.performRollout();
    }
    const actionProbabilities = analyzer.getActionProbabilitiesWithExtraEntry();
    actionProbabilities.setLastEntry(analyzer.getActivePlayer());
    positions.push({
      gameState: analyzer.getGameStateVector(),
      legalActions: analyzer.getActionFilter(),
      actionProbabilitiesAndValue: actionProbabilities,
    });
    analyzer.selectAction(TEMPERATURE);
  }

  const winner = analyzer.expectWinner();

  for (const position of positions) {
    position.actionProbabilitiesAndValue.setLastEntry(
      winner === position.actionProbabilitiesAndValue.lastEntry()
    );
  }
...
interface MctsAnalyzer {
  reset(state: GameState): void;

  isGameOver(): boolean;
  performRollout(): void;
  getActionProbabilitiesWithExtraEntry(): Matrix;
  getActivePlayer(): Player;
  getGameStateVector(): Matrix;
  getActionFilter(): boolean[];
  selectAction(temperature: number): boolean;
  expectWinner(): Player;
...
interface MctsAnalyzerInternalData {
...
function getMctsAnalyzer(
  ...
  return {
    ...
    isGameOver,
    performRollout,
    getActionProbabilitiesWithExtraEntry,
    getActivePlayer,
    getGameStateVector,
    getActionFilter,
    selectAction,
  ...
  function reset(state: GameState): void {
    heap.fill(0, 0, mallocIndex);
    rootNodeIndex = 0;
    mallocIndex = 22;

    const { currentBoard, pendingAnimalStep } = state;
    const turnNumber = 1;
  ...
  function isGameOver(): boolean {
    const optWinner = getImmediateWinner(heap, rootNodeIndex);
    return optWinner !== NO_WINNER;
  }
  ...
  function performRollout(): void {
    ...
    // if (getRollouts(leafIndex) === 0 && leafIndex !== rootNodeIndex) {
  ...
  function getRootPointer(): NodePointer {
    return indexToPointer(rootNodeIndex);
  }
  ...
  function getIndexOfChildWithBestAtomic(): number {
    const rootActivePlayer =
      (heap[rootNodeIndex + NodeOffsets.SnipeSetsTurnNumberAndMovedAnimal] >>>
        16) &
      0b1;
    const rootInactivePlayer = ~rootActivePlayer & 0b1;

    const rootChildListStartIndex =
      heap[rootNodeIndex + NodeOffsets.ChildListStartIndex];
