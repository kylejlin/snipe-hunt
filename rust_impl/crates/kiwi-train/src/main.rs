...
const DEFAULT_RUN_DIR: &str = "training/kiwi-main";
...
const REPLAY_MAGIC: &[u8; 8] = b"KIWRPL01";
...
fn run() -> io::Result<bool> {
    ...
    match command {
        ...
        "evaluate" => {
            let against = option(&arguments, "--against").ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidInput,
                    "evaluate requires --against CHECKPOINT",
                )
            })?;
            let pairs = parse_option(&arguments, "--pairs", 24)?;
            let simulations = parse_option(&arguments, "--simulations", DEFAULT_SIMULATIONS)?;
            evaluate(&run_dir, Path::new(against), pairs, simulations)?;
            Ok(true)
        }
        "publish" => {
            publish(&run_dir)?;
        ...
        other => Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            format!("unknown command {other:?}; use `kiwi-train help`"),
        )),
    }
}
...
fn help() {
    println!(
        "Kiwi rules-only self-play trainer\n\
         \n\
         ...
         The latest network continuously generates self-play; training has no champion\n\
         gate, promotion arena, or automatic rollback. The evaluate command is optional\n\
         measurement only and never changes a run.\n\
         Timestamped progress reports are on by default and print every 500 games.\n\
         Ctrl+C requests a graceful stop after the current self-play batch and\n\
...
impl ProgressSnapshot {
    fn from_run(run: &RunState) -> Self {
        Self {
            games: run.games,
            training_steps: run.model.training_steps,
...
fn format_start_report(
    ...
    format!(
        "Kiwi continuous training resumed at game {}, network step {}, with {} replay positions. Base simulations/action: {}; workers: {}; run directory: {}.",
        format_count(snapshot.games),
        format_count(snapshot.training_steps),
...
fn format_periodic_report(
    ...
    format!(
        "Kiwi is training continuously through game {}. Network step {}; replay contains {} positions; latest loss {:.4}. {latest_game}",
        format_count(snapshot.games),
        format_count(snapshot.training_steps),
        format_count(snapshot.replay_positions as u64),
        last_loss,
...
fn format_completion_report(snapshot: ProgressSnapshot) -> String {
    format!(
        "Training window complete at game {}. Network step {}; replay contains {} positions.",
        format_count(snapshot.games),
        format_count(snapshot.training_steps),
        format_count(snapshot.replay_positions as u64),
    )
...
fn format_shutdown_report(snapshot: ProgressSnapshot, run_dir: &Path) -> String {
    format!(
        "Graceful shutdown complete at game {}. Network step {}; replay contains {} positions. A full resumable checkpoint was saved in {}.",
        format_count(snapshot.games),
        format_count(snapshot.training_steps),
        format_count(snapshot.replay_positions as u64),
        run_dir.display(),
...
fn train(
    ...
    while started.elapsed() < duration && !shutdown_requested() {
        ...
        thread::scope(|scope| -> io::Result<()> {
            ...
            for game in receiver {
                ...
                if run.games % SNAPSHOT_INTERVAL == 0 {
                    archive_snapshot(run_dir, &run)?;
                }
                if run.games % PROGRESS_REPORT_INTERVAL == 0 {
                    print_progress(
                        progress_reports,
    ...
    save_run(run_dir, &run, last_loss, true)?;
...
fn install_shutdown_handler() -> io::Result<()> {
    ...
    ctrlc::set_handler(|| {
        if SHUTDOWN_REQUESTS.fetch_add(1, Ordering::SeqCst) == 0 {
            eprintln!(
                "\nGraceful shutdown requested. Kiwi will finish the current self-play batch \
                 and then save a full checkpoint. Press Ctrl+C again to force an immediate \
                 exit without saving."
            );
        } else {
...
fn self_play(model: &Model, deal_seed: u64, simulations: usize, rng: &mut Rng) -> SelfPlayGame {
    ...
    for action_number in 0..MAX_ATOMIC_ACTIONS {
        ...
        state = state
            .apply(action)
            .expect("Kiwi self-play selected a legal action");
...
fn evaluate(run_dir: &Path, against: &Path, pairs: usize, simulations: usize) -> io::Result<()> {
    let run = load_run(run_dir)?;
    let reference = Model::load(against)?;
    ...
    println!(
        "latest step {} vs reference step {} from {}: score={:.3}, lower95={:.3}, games={}",
        run.model.training_steps,
        reference.training_steps,
        against.display(),
        result.score,
        result.lower_95,
        result.games
...
fn status(run_dir: &Path) -> io::Result<()> {
    ...
    println!("training_protocol=continuous-v1");
...
fn publish_to(run_dir: &Path, destination: &Path, web_directory: &Path) -> io::Result<()> {
        ...
        .any(|line| line == "purity=rules-only-fresh-seed")
    {
        return Err(io::Error::other(
            "run is missing Kiwi's rules-only fresh-seed purity marker",
        ));
    }
    if !state
        .lines()
        .any(|line| line == "training_protocol=continuous-v1")
    {
        return Err(io::Error::other(
            "run is missing Kiwi's continuous-training protocol marker",
        ));
    }
    ...
    let expected_steps = read("training_steps")
        .ok_or_else(|| io::Error::other("run metadata is missing training_steps"))?;
    let source = run_dir.join("latest.bin");
    let latest = Model::load(&source)?;
    if latest.training_steps != expected_steps {
        return Err(io::Error::other(format!(
            "latest checkpoint step {} does not match run metadata step {expected_steps}",
...
fn load_meta(path: &Path) -> io::Result<Metadata> {
    ...
    {
        return Err(io::Error::other(
            "run is not marked with Kiwi's continuous-training protocol",
        ));
    }
    let read = |name: &str| {
        text.lines()
...
fn save_run(run_dir: &Path, run: &RunState, loss: f32, save_replay: bool) -> io::Result<()> {
    ...
    let metadata = format!(
        "purity=rules-only-fresh-seed\ntraining_protocol=continuous-v1\ngames={}\nrng={}\ntraining_steps={}\nreplay_positions={}\nlast_loss={loss}\nupdated_unix={}\n",
        run.games,
        run.rng.0,
        run.model.training_steps,
        run.replay.len(),
...
fn archive_snapshot(run_dir: &Path, run: &RunState) -> io::Result<()> {
    let snapshots = run_dir.join("snapshots");
    fs::create_dir_all(&snapshots)?;
    atomic_write(
        &snapshots.join(format!(
            "network-step-{}-game-{}.bin",
            run.model.training_steps, run.games
        )),
        &run.model.to_bytes(),
    )
}
...
fn load_replay(path: &Path) -> io::Result<ReplayBuffer> {
    ...
    if &magic != REPLAY_MAGIC {
        return Err(invalid("invalid Kiwi replay"));
    }
    let count = read_u64(&mut reader)?;
    if count > MAX_REPLAY as u64 {
        return Err(invalid("Kiwi replay exceeds configured capacity"));
    }
    ...
    for _ in 0..count {
        ...
        if policy_len == 0 || policy_len > ACTION_SIZE {
            return Err(invalid("invalid Kiwi replay policy length"));
        }
        ...
        for _ in 0..policy_len {
            ...
            if usize::from(index) >= ACTION_SIZE || weight == 0 || seen[usize::from(index)] {
                return Err(invalid("invalid sparse Kiwi replay policy"));
            }
        ...
        if !(-1..=1).contains(&value) {
            return Err(invalid("invalid Kiwi replay outcome"));
        }
    ...
    if reader.read(&mut trailing)? != 0 {
        return Err(invalid("Kiwi replay contains trailing bytes"));
    }
    Ok(replay)
}
...
mod tests {
    ...
    fn progress_reports_use_readable_utc_timestamps_and_counts() {
        ...
        assert_eq!(
            ...
            "[2001-09-09T01:46:40Z] Kiwi is training."
        );
    }

    #[test]
    fn progress_reports_describe_one_continuously_updated_network() {
        let report = format_periodic_report(
            progress_snapshot(160_000, 492_000),
            1.25,
            Some(Player::Alpha),
            83,
        );
        assert!(report.contains("training continuously through game 160,000"));
        assert!(report.contains("Network step 492,000"));
        assert!(!report.contains("champion"));
        assert!(!report.contains("promotion"));
        assert!(!report.contains("arena"));
    }

    #[test]
    fn publish_exports_the_latest_continuous_network() {
    ...
    #[test]
    fn publish_rejects_a_gated_run_without_the_continuous_marker() {
        let root = test_directory("publish-gated");
        let run_dir = root.join("run");
        fs::create_dir_all(&run_dir).unwrap();
        ...
        assert!(
            error
                .to_string()
                .contains("continuous-training protocol marker")
        );
        fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn checkpoints_contain_no_champion_or_promotion_state() {
        let root = test_directory("continuous-checkpoint");
        fs::create_dir_all(&root).unwrap();
        let run = RunState {
            model: Model::seeded(INITIAL_SEED),
            optimizer: Adam::new(),
        ...
        save_run(&root, &run, 0.0, true).unwrap();

        let state = fs::read_to_string(root.join("state.txt")).unwrap();
        assert!(state.contains("training_protocol=continuous-v1"));
        assert!(!state.contains("champion"));
        assert!(!state.contains("promotion"));
        assert!(!root.join("champion.bin").exists());
        assert!(!root.join("arena.csv").exists());
        fs::remove_dir_all(root).unwrap();
    }
    ...
    fn progress_snapshot(games: u64, training_steps: u64) -> ProgressSnapshot {
        ProgressSnapshot {
            games,
            training_steps,
